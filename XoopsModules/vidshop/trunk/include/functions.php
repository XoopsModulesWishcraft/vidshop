<?php
	...
	function saveEditVideo($video_id)
	{
		...
		$video->setVar('currency', $_POST["curr"]);
		...
		if ($videoHandler->insert($video)) {
			...
			redirect_header('admin.php', 4, sprintf(_VSP_RH_VIDSHOP_NOCREATION, $video->getVar('name'), $video->getErrors()));
			exit(0);		
		}
	}
	...
	function saveEditVideoCategory($cid)
	{
		...
		if ($cid = $videoCatHandler->insert($video)) {				
			...
			redirect_header('admin.php', 4, sprintf(_VSP_RH_CATEGORY_NOCREATION, $video->getVar('name'), $video->getErrors()));
			exit(0);		
		}
	}
	...
	function saveNewVideoCategory()
	{
		...
		if ($cid = $videoCatHandler->insert($video)) {				
			...
			redirect_header('index.php', 4, sprintf(_VSP_RH_CATEGORY_NOCREATION, $video->getVar('name')));
			exit(0);		
		}
	}
