...
class VidshopFormPaypalButton extends XoopsFormElement
{
    ...
    function render()
    {
		...
		$items = $cartItemsHandler->getAll($criteria);
		...
		$ret .= '<input type="hidden" name="currency_code" value="GBP">';
		...
		foreach($items as $item) {
			...
			$ret .= '<input type="hidden" name="item_name_'.$ii.'" value="'.$video->getVar('name').'">';
			$ret .= '<input type="hidden" name="amount_'.$ii.'" value="'.$video->getVar('price').'">';
			$ret .= '<input type="hidden" name="currency_code_'.$ii.'" value="'.$video->getVar('currency').'">';
			$ret .= '<input type="hidden" name="quantity_'.$ii.'" value="1">';	
		}
		$ret .= '<input type="submit" value="Make Payment >>" border="0" name="I1">';
		return $ret;
	}
}
